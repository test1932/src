...
class koishi(abstractCharacter):
    ...
    def __init__(self, gameObj) -> None:
        super().__init__(
            "assets/images/characters/yukari/sheet.png", 
            "assets/images/characters/yukari/icon.png",
            gameObj, 600, 135, 52)        
        
        parts = parseSpriteSheet(self.sheetPath)
        ...
        self.characterTransitions = {
            koishi.MELEE_BACK: abstractCharacter.IDLE,
            koishi.MELEE_FORWARD: abstractCharacter.IDLE,
            koishi.MELEE_UP_START: koishi.MELEE_UP_LOOP,
            koishi.MELEE_UP_END: abstractCharacter.IDLE,
            koishi.MELEE_DASH_FORWARD: abstractCharacter.IDLE,
            koishi.MELEE_DOWN: abstractCharacter.IDLE
        }
        
        self.setFrames(self.framePoints, self.framePoints[0][0][0][1][0] - self.framePoints[0][0][0][0][0])
        if not koishi.isSetup:
            koishi.isSetup = True
            koishi.loadProjectileImgs()
    ...
    def forwardMelee(self, spellaction, player, isCombo = False):
        #check if in air
        player.setAnimation(koishi.MELEE_FORWARD)
    ...
    def downMelee(self, spellaction, player):
        player.setAnimation(koishi.MELEE_DOWN)
        player.setXVelocity(0)
        player.setYVelocity(0)
        player.setStun(0.1)
    ...
    def upMelee(self, spellaction, player):
        ...
        player.setAnimation(koishi.MELEE_UP_START)
        player.setXVelocity(0)
        player.setYVelocity(0)
        time.sleep(1)
        player.setAnimation(koishi.MELEE_UP_END)
        player.setStun(0.1)
    ...
    def __baseMelee(self, spellaction, player):
        player.setAnimation(koishi.MELEE_CLOSE)
        player.setStun(0.5)
