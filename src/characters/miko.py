...
class miko(abstractCharacter):
    ...
    def __init__(self, gameObj) -> None:
        ...
        self.characterTransitions = {
            miko.MELEE_BACK: abstractCharacter.IDLE,
            miko.MELEE_FORWARD: abstractCharacter.IDLE,
            miko.MELEE_UP_START: miko.MELEE_UP_LOOP,
            miko.MELEE_UP_END: abstractCharacter.IDLE,
            miko.MELEE_DASH_FORWARD: abstractCharacter.IDLE,
            miko.MELEE_DOWN: abstractCharacter.IDLE
        }
        
        self.setFrames(self.framePoints, self.framePoints[0][0][0][1][0] - self.framePoints[0][0][0][0][0])
        if not miko.isSetup:
            miko.isSetup = True
            miko.loadProjectileImgs()
    ...
    def forwardMelee(self, spellaction, player, isCombo = False):
        #check if in air
        player.setAnimation(miko.MELEE_FORWARD)
    ...
    def downMelee(self, spellaction, player):
        player.setAnimation(miko.MELEE_DOWN)
        player.setXVelocity(0)
        player.setYVelocity(0)
        player.setStun(0.1)
    ...
    def upMelee(self, spellaction, player):
        ...
        player.setAnimation(miko.MELEE_UP_START)
        player.setXVelocity(0)
        player.setYVelocity(0)
        time.sleep(1)
        player.setAnimation(miko.MELEE_UP_END)
        player.setStun(0.1)
    ...
    def __baseMelee(self, spellaction, player):
        player.setAnimation(miko.MELEE_CLOSE)
        player.setStun(0.5)
