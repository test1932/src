...
from effects.speedBoost import dashBoost
...
class yukari(abstractCharacter):
    ...
    def setup(cls):
        ...
        cls.spritesheet = pygame.image.load(
            abstractCharacter.pathTo(cls, "sheet.png")).convert_alpha()
        cls.icon = pygame.image.load(
            abstractCharacter.pathTo(cls, "icon.png")).convert_alpha()
        cls.portraits = pygame.image.load(
            abstractCharacter.pathTo(cls, "portraits.png")).convert_alpha()
    ...
    def __init__(self, gameObj) -> None:
        super().__init__(gameObj, 300, 135, 52)
    ...
    def dashForward(self, player):
        player.addEffect(dashBoost(player, 600, -1 if player.isFacingLeft() else 1))
